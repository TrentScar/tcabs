		...
		if(!$_SESSION['loggedUser']->uRoles['convenor']) {
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST') {
				if(isset($_POST['submit'])) {
					...
					} else if($_POST['submit'] == 'editProject') {
...
<html lang="en">
  ...
  <body class="loggedin">
		...
		<div class="content">
			<h2>Project Administration</h2><h2-date><?php echo date('d F, Y (l)'); ?></h2-date><br>
		<div>
		...
		<ul class="nav nav-tabs">
  		<li class="nav-item">
    		<a class="nav-link <?php if((isset($_POST['submit']) && $_POST['submit'] == 'addProject') || $_SERVER['REQUEST_METHOD'] == 'GET') { echo 'active';} ?>" data-toggle="tab" href="#home">Add</a>
  		</li>
  		<li class="nav-item>">
		...
		<div class="tab-content">
			...
			<div>
				<form action="registerProject.php" method="post" class="was-validated"><br>
					<p class="h4 mb-4 text-center">Edit Project (<?php echo ($_POST['update']); ?>)</p>
					<input type="text" id="projName" name="projName" class="form-control" value="<?php echo $value['projName']; ?>"  required><br>
					<input type="text" id="projDesc" name="projDesc" class="form-control" value="<?php echo $value['projDesc']; ?>" required><br>
					<button class="btn btn-info my-4 btn-block" type="submit" name="submit" value="editProject">Edit Project</button>
				</form>
			</div>
			<?php  } ?>
			<?php  } ?>
  		...
  		<div class="tab-pane container <?php if((isset($_POST['submit']) && $_POST['submit'] == 'addProject') || $_SERVER['REQUEST_METHOD'] == 'GET') { echo 'active show';} ?>" id="home">
				<form action="registerProject.php" method ="post" class="was-validated"><br/>
  	  		<p class="h4 mb-4 text-center">Register Projects</p>
					<input type="text" id="projName" name="projName" class="form-control" required><br>
					<input type="text" id="projDesc" name="projDesc" class="form-control" required><br>
  				<button class="btn btn-info my-4 btn-block" type="submit" name="submit" value="addProject">Add Project</button>
				</form>
			</div>
  		...
  		<div class="tab-pane container fade <?php if(isset($_POST['submit']) && $_POST['submit'] == 'bulkImport') { echo 'active show';} ?>" id="menu1">
  	  		...
  	  		<p class="h4 mb-4 text-center">Bulk Import</p>
  				...
  				<button class="btn btn-info my-4 btn-block" type="submit" name="submit" value="bulkImport">Add Projects</button>
				</form>
			</div>
